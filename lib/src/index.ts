...
import { sendEmailOptionsSchema } from "./schema";
import { EmailContact, SendEmailOptions, UsendOptions } from "./usend";
...
export class Usend {
  ...
  constructor(options?: UsendOptions) {
    ...
    this.vendorApiKey =
      options?.vendorApiKey || process.env.USEND_VENDOR_API_KEY;
  ...
  public async sendEmail(options: SendEmailOptions): Promise<void> {
    try {
      ...
      const errors = (error as ZodError).errors.map(
    ...
    const domain = this.domain || this.extractDomain(options.from);
...
export function replaceHtml(
  template: string,
  dictionary: { [key: string]: string | number }
): string {
